...
- **Document**: write resolved metadata claims into Akoma Ntoso body XML during `save()` (title, date, court, jurisdiction, case number, categories; not parties or identifiers)
- **Metadata**: parties are not written back to `uk:party` on `save()`; legacy body parties are still read pre-materialisation and imported into claims during save
