...
class TestAdvancedSearch(unittest.TestCase):
    ...
    def test_invoke_called_with_default_params_when_optional_parameters_not_provided(
        ...
        """
        Scenario: Searching with only the q parameter
        Given a client instance
        When the advanced_search method is called with only the q parameter
        Then it should call the MarkLogic module with the q parameter and
            defaults for the rest and return the response
        """

        with patch.object(self.client, "invoke"):
            response = self.client.advanced_search(q="test query")
    ...
    def test_error_raised_when_invoke_raises_an_error(self):
        """
        Scenario: MarkLogic error while searching
        Given a client instance
        When the advanced_search method is called and MarkLogic returns an error
        Then it should raise an exception with the error message
        """
        error_message = "Error message from MarkLogic"
        with patch.object(self.client, "invoke"):
            self.client.invoke.side_effect = Exception(error_message)
